...
const Approach = () => {
  ...
  useScrollReveal(sectionRef, { stagger: 0.11 });

  return (
    <section ref={sectionRef} id="approach" data-ambient="cyan" className="site-section approach-section border-b section-rule py-20 md:py-32">
      <div className="page-canvas">
        <div data-reveal className="section-heading grid gap-6 border-b section-rule pb-7 md:grid-cols-2 md:items-end">
          <div data-reveal-item><p className="section-label text-[#71717a]">02 / Working model</p><h2 className="font-display mt-3 text-3xl font-medium tracking-[-0.02em] md:text-[46px]">A project with VDG.</h2></div>
          <p data-reveal-item className="max-w-lg text-[#a1a1aa]">A straightforward way to move from an open question to work that can be owned and used.</p>
        </div>

        <ol data-reveal className="working-rail">
          {stages.map((stage, index) => {
            const selected = index === activeIndex;
            return <li key={stage.name} data-reveal-item className={`working-stage ${selected ? "working-stage--selected" : ""}`} onMouseEnter={() => setActiveIndex(index)}>
